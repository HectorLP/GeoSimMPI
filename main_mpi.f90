...
!**********************************************************
  ...
  !  Get Number of compound bars from maxv.out
      ...
      ! ENDIF
        ...
        ! IF (nc_determine > 0) THEN
        ncbar_process(iproc+1) = ncbar_partition1
        IF (iproc < nc_determine) THEN
            ncbar_process(iproc+1) = ncbar_partition1 + 1
        ENDIF
  ...
  ! 			kk = iproc * ncbar_partition + i_partition
  ! 			IF (kk<=nocb) THEN
  ! 				WRITE(*,*) 'The index for the component bar in arcgen is',kk
  ! 				zzi = 0.0
  ! 				xxi = 0     
  ! 				!kk = iproc+1 !Naum      
  ! 				wa = (2*wcb(kk))/5.0                                              !WSU, 4/4/08                                                     
  ...
  ! 				l = 0
  ! 				m = 1 
  ! 				zzi(m,i_partition) = zstart 
  ! 		! Loop with each layer
  ! 			  k = 0
  ! 			  !DO i_test = 1, mcb
  ! 			  !    xxx(i_test, i_partition) = 1.0
  ! 			  !END DO
  ! 			DO WHILE (zzi(m,i_partition) > zx)
  ! 				WRITE(*,*) 'This is a proof for calculating the arc points ',m
  ! 				k = k+1
  ! 				yii = yi
  ! 				lx = xi-wa/2.0                                                  !WSU, 4/4/08                                                
  ! 				rx = xi+wa/2.0                                                  !WSU, 4/4/08
  ! 				ninfl = 2                                                       !WSU, 4/4/08
  ! 				xxx(m,i_partition) = xi
  ! 				yyy(m,i_partition) = yi
  ! 				idir = 0                                                         !WSU 05-13-08
  ! 				il = 0                                                             !WSU 06-19-08
  ! 				ir = 0                                                            !WSU 06-19-08
  ! 	   !10   IF (xxx(m) >= lx)THEN
  ! 			10	DO WHILE (xxx(m,i_partition) >= lx)
  ! 					il = il+1                                                       !WSU 06-19-08 
  ! 					IF(xxx(m,i_partition) == xi)THEN
  ! 						iiub(m,i_partition) = 1
  ! 					ELSE
  ! 						iiub(m,i_partition) = 2
  ! 					END IF
  ! 			!21     CONTINUE
  ...
  ! 						CALL norgen(h,humn,huvar,is1,is2,is3,jtest)
  ! 					END DO
  ! 					wu(m,i_partition) = w !70.0
  ! 					lu(m,i_partition) = l !300.0
  ! 					hhu(m,i_partition) = h !1.5
  ! 					IF (m == 1)THEN
  ! 						x1(m,i_partition) = xxx(m,i_partition)
  ! 						y1(m,i_partition) = yyy(m,i_partition)
  ...
  ! 						ENDIF                                                           !WSU 06-19-08
  ! 						xx = lu(m,i_partition)/TAN(ango)                                             !WSU 06-19-08 
  ! 						IF (idir==1)THEN                                              !WSU 05-13-08
  ! 							x2(m,i_partition) = xxx(m,i_partition)+xx                                             !WSU 05-13-08
  ! 						ELSE                                                          !WSU 05-13-08
  ! 							x2(m,i_partition) = xxx(m,i_partition)-xx
  ! 						ENDIF                                                         !WSU 05-13-08
  ! 						y2(m,i_partition) = yyy(m,i_partition)+lu(m,i_partition)                                          !wsu 4/4/08
  ! 				!          x3(m) = xxx(m)                                               !wsu 4/4/08
  ! 				!          y3(m) = yyy(m)+lu(m)                                         !wsu 4/4/08
  ! 						ap(m,i_partition) = -1/atii                                                !WSU 06-19-08
  ! 						bp(m,i_partition) = y1(m,i_partition)-ap(m,i_partition)*x1(m,i_partition)
  ...
  ! 						zzi(m,i_partition) = zstart
  ! 						m = m+1
  ! 						xxx(m,i_partition) = xxx(m-1,i_partition)-(wu(m-1,i_partition)/2.0)
  ! 						CALL norgen(yy,yymn,yyvar,is1,is2,is3,jtest)
  ! 				!          WRITE(1556,*)m
  ! 				!          yy = 8.0
  ! 						yyy(m,i_partition) = yyy(m-1,i_partition)-yy
  ! 					ELSE
  ! 						wm(m,i_partition) = wu(m,i_partition)
  ! 						IF (wm(m,i_partition) < wm(m-1,i_partition))THEN
  ! 							wm(m,i_partition) = wm(m-1,i_partition)
  ! 						ENDIF
  ! 							lm(m,i_partition) = lu(m,i_partition)
  ! 						IF (lm(m,i_partition) < lm(m-1,i_partition))THEN
  ! 							lm(m,i_partition) = lm(m-1,i_partition)
  ! 						ENDIF
  ! 				!98     CONTINUE
  ! 						x1(m,i_partition) = xxx(m,i_partition)
  ! 						y1(m,i_partition) = yyy(m,i_partition)
  ...
  ! 					ENDIF
  ! 				END DO
  ! 				IF (xxx(m,i_partition) < lx) THEN 
  ! 					xxx(m,i_partition) = xi+(wu(m-1,i_partition)/2.0)
  ! 					yyy(m,i_partition) = yi-yy
  ! 			!50     IF (xxx(m) <= rx)THEN
  ! 					DO WHILE (xxx(m,i_partition) <= rx)
  ! 						ir =ir+1                                                    !WSU 06-19-08
  ! 						iiub(m,i_partition) = 3
  ! 			!51       CONTINUE
  ! 						CALL norgen(w,wumn,wuvar,is1,is2,is3,jtest)
  ! 						!IF (w <= 0.0) GOTO 51
  ! 						DO WHILE (w <= 0.0)
  ...
  ! 								atii = -(atii)                                            !WSU 06-19-08
  ! 							ENDIF
  ! 							xx = lu(m,i_partition)/TAN(ango)                                        !WSU 05-13-08
  ! 							IF (idir==1)THEN                                           !WSU 05-13-08
  ! 								x2(m,i_partition) = xxx(m,i_partition)-xx                                         !WSU 05-13-08
  ! 							ELSE                                                       !WSU 05-13-08
  ! 								x2(m,i_partition) = xxx(m,i_partition)+xx
  ...
  ! 						ENDIF
  ! 					END DO
  ! 					IF (xxx(m,i_partition) > rx) Then
  ! 						IF (yyy(m,i_partition) < (lc-lumn/2.0)) THEN                          !WSU, 4/4/08                               
  ! 							IF (yi < lc/4.0)THEN                                     !WSU, 5/13/08
  ! 								yyy(m,i_partition) = yi+lu(m-1,i_partition)/2.0
  ! 							ELSE
  ! 								yyy(m,i_partition) = yi+lu(m-1,i_partition)/3.5
  ! 							END IF
  ! 							IF (yyy(m,i_partition) <=l1) THEN                                    !WSU, 4/4/08
  ! 								xa = ((wb/2.0-wa/2.0)/l1)*yyy(m,i_partition)                               !WSU, 4/4/08
  ! 								IF ((zzi(m,i_partition)-z(kk)) < hcb(kk))THEN                              !WSU, 4/4/08
  ...
  ! 							ELSE                                                         !WSU, 4/4/08
  ! 								idir = 1                                                     !WSU 06-19-08
  ! 								IF (yyy(m,i_partition) <=(l1+l2)) THEN                                    !WSU, 4/4/08
  ! 									ya = (l1+l2)-yyy(m,i_partition)                                           !WSU, 4/4/08
  ...
  ! 									ENDIF                                                      !WSU, 4/4/08
  ! 								ELSE                                                        !WSU, 4/4/08
  ! 									ya = yyy(m,i_partition)-(l1+l2)                                           !WSU, 4/4/08
  ...
  ! 				zzi(m,i_partition) = zstart-(hhu(m-1,i_partition)/2.0)                             !WSU, 4/4/08
  ! 				zstart = zzi(m,i_partition)
  ! 				yi = yii
  !         !GOTO 9
  ! 			END DO
  ...
  ! 	  IF (tmp_noub > noub) THEN
  ! 		noub = tmp_noub
  ! 	  END IF
  !       noub = m-1
  !       na =2                                                     !WSU 06-19-08
      
  ! 	  IF (wm(m-1,i_partition)>wm_max) THEN
  ! 		wm_max = wm(m-1,i_partition)
  ! 	  END IF
      
  ! 	  IF (lm(m-1,i_partition)>lm_max) THEN
  ! 		lm_max = lm(m-1,i_partition) 
  ! 	  END IF
      
  ! 	  ALLOCATE (xinfl_1(tmp_noub,na),STAT=IERR)
  ! 	  ALLOCATE (yinfl_1(tmp_noub,na),STAT=IERR)
  ! 	  ALLOCATE (a_1(tmp_noub,na),STAT=IERR)
  ! 	  ALLOCATE (b_1(tmp_noub,na),STAT=IERR)
  ! 	  ALLOCATE (width_1(tmp_noub),STAT=IERR)
  ! 	  ALLOCATE (length_1(tmp_noub),STAT=IERR)
  ! 	  ALLOCATE (hu_1(tmp_noub),STAT=IERR)
  ! 	  ALLOCATE (zi_1(tmp_noub),STAT=IERR)
  ! 	  ALLOCATE (iub_1(tmp_noub),STAT=IERR)
    
  !       WRITE(intform(3:3),'(I1)') icount_digits(kk-1)
  ...
  !         OPEN(15,FILE=filename,FORM='formatted',STATUS='unknown')
  !       ENDIF
  ! 	!Do i_ncbar = 1,ncbar_partition
  !       DO n = 1,noub_array(kk)
  ! 		xinfl_1(n,1)=x1(n,i_partition)
  ! 		xinfl_1(n,2)=x2(n,i_partition)
  ! 		yinfl_1(n,1)=y1(n,i_partition)
  ! 		yinfl_1(n,2)=y2(n,i_partition)
  ! 		a_1(n,1)=ap(n,i_partition)
  ! 		b_1(n,1)=bp(n,i_partition)
  ! 		width_1(n)=wu(n,i_partition)
  ! 		length_1(n)=lu(n,i_partition)
  ! 		hu_1(n)=hhu(n,i_partition)
  ! 		zi_1(n)=zzi(n,i_partition)
  ! 		iub_1(n)=iiub(n,i_partition)
  !       END DO
  ! 	!END DO
  ! 	  IF (ibin == 1) THEN
  ! 		WRITE(15) xinfl_1(1:tmp_noub,1:na)
  ! 		WRITE(15) yinfl_1(1:tmp_noub,1:na)
  ! 		WRITE(15) a_1(1:tmp_noub,1:na)
  ! 		WRITE(15) b_1(1:tmp_noub,1:na)
  !         WRITE(15) length_1(1:tmp_noub)
  ! 		WRITE(15) width_1(1:tmp_noub)
  ! 		WRITE(15) hu_1(1:tmp_noub)
  ! 		WRITE(15) zi_1(1:tmp_noub)
  ! 		WRITE(15) iub_1(1:tmp_noub)
  !       ELSE
  ! 		WRITE(15) xinfl_1(1:tmp_noub,1:na)
  ! 		WRITE(15) yinfl_1(1:tmp_noub,1:na)
  ! 		WRITE(15) a_1(1:tmp_noub,1:na)
  ...
  !       ENDIF
  ! 	  CLOSE(15)
  ! 	!release local memory after saving the data of each component bar 
  ...
  ! 			WRITE(*,*) 'Finish the calculation for the argen subroutine',kk
  ...
  !       CALL MPI_AllReduce(wm_max,wxub,1,MPI_REAL,MPI_MAX, &
  !                         MPI_COMM_WORLD,ierr)
  !       adum = 'wxub-mpi'
  !       CALL CHECK(adum,ierr)
  
  !      CALL MPI_AllReduce(lm_max,mxlub,1,MPI_REAL,MPI_MAX, &
  !                         MPI_COMM_WORLD,ierr)
  !      adum = 'mxlub-mpi'
  !      CALL CHECK(adum,ierr)     
  !      WRITE(55,*)maxUB,wxub,mxlub
  ! 	 !wm(m-1),lm(m-1)
  ...
  !     END SUBROUTINE arcgen
      ...
      ! wrong.
         ...
         WRITE(*,*) 'The max wub is ', wm(m-1), wxub
         WRITE(*,*) 'The max lub is ', lm(m-1), mxlub
         CALL CHECK(adum,ierr)     
  ...
  !  Will not work if more than one compound bar per processor
      ...
      DO i_partition = 0,ncbar_partition-1
        ...
        IF (iiii < nocb) THEN
          ...
          filename2(1:11) = 'zhpara.out.'
          WRITE(filename2(12:),intform) n_partition
          OPEN(151,FILE=filename1,FORM='unformatted',STATUS='unknown')
          ...
          igl=0
          !      
          WRITE(*,*) 'Start to build files for output in process ', n_partition
          IF(igl == 1) THEN
            WRITE(intform(3:3),'(I1)') icount_digits(iiii)
            filename(1:6) = 't.out.'
      ...
      !  These files are used as check on linegen; eliminate in large run
          ...
          IF (ichk == 1) THEN
            filename(1:11) = 'length1.out.'
      !        kproc=TRIM(iproc)
            WRITE(filename(13:),intform)n_partition
      !        write(8732,*)filename
            IF(ibin == 1) THEN
              OPEN(16,FILE=filename,FORM='unformatted',STATUS='unknown')
            ELSE
  ...
  !  Read in the information about width and length of unit bars.
          !WRITE(*,*) 'Allocate the memory for array in lingen in ', n_partition
          WRITE(*,*) 'The arrays are for component bar ', n_partition
          ncbar_partition_1 = 1
  ...
  !        OPEN(718,FILE=filename,FORM='formatted',STATUS='unknown') 
          WRITE(*,*) 'Start the computation for the unit bar ', noub_array(n_partition+1)
          WRITE(*,*) 'Finish the allocation for component bar ', n_partition
          jtest = 0
          itest=0 !0
          ii = 1
          IF (itest == 1) THEN
      ...
      !vf       endif
            END DO
          END DO
          WRITE(*,*) 'Finish the calculation after allocation in ', n_partition
          IF (iiii == 0)WRITE(*,*)'Finished generating angle decrements'
    ...
    ! Determine each coset line angle
          ...
          loopCB:  DO ij=1,ncbar_partition_1        
            mcb1=0
      !        mxlub=300.0
      !        wxub=70.0
      ...
      !  dcy is then used to offset the coset line.
            ...
            END DO  loopUB
          END DO  loopCB 
          WRITE(*,*) 'Finish second calculation of allocation ', n_partition
          ttw=0.0
      261   DO k =1,nlines
            txw(k)=twm
            ttw = txw(k)+ttw
            ...
            txc=txmax(k)
          END DO   
          WRITE(*,*) 'Finish third calculation of allocation ', n_partition
      262   DEALLOCATE(tymin, STAT=IERR)
          ...
          adum = 'thi-deal'
          CALL CHECK(adum,ierr)
          WRITE(*,*) 'Finish the first part computation component bar ', n_partition
  ...
  ! Initialize for calculating proportions of textures for troughs
      ...
      ! Comment this out for large runs
                ...
                IF (ichk == 1) THEN
                  IF (ibin == 1) THEN
                    ...
                    WRITE(15)llen(i,ii)
                  ELSE
                    WRITE(12,'(3(I4,2X),1PE13.6)')n_partition+1,i,ii,cthi(ij,i,ii)
                    WRITE(15,'(3(I4,2X))')n+1,i,ii,n
                    WRITE(15,*)avymin(i,ii),avymax(i,ii)
                    WRITE(15,*)avzmin(i,ii),avzmax(i,ii)
                    WRITE(15,*)llen(i,ii)
                  ENDIF
      ...
      !  Allocate new arrays for write
          ...
          ALLOCATE(slopeN(ncbar_partition_1,jcm), STAT=IERR)
          adum = 'slopeN'
          CALL CHECK(adum,ierr)
  
          ALLOCATE(yintN(ncbar_partition_1,jcm), STAT=IERR)
          adum = 'yintN'
          CALL CHECK(adum,ierr)
  
          ALLOCATE(xminN(ncbar_partition_1,jcm), STAT=IERR)
          adum = 'xminN'
          CALL CHECK(adum,ierr)
  
          ALLOCATE(xmaxN(ncbar_partition_1,jcm), STAT=IERR)
          adum = 'xmaxN'
          CALL CHECK(adum,ierr)
  
          ALLOCATE(cyminN(ncbar_partition_1,jcm), STAT=IERR)
          adum = 'cyminN'
          CALL CHECK(adum,ierr)
  
          ALLOCATE(czmaxN(ncbar_partition_1,jcm), STAT=IERR)
          adum = 'cymaxN'
          CALL CHECK(adum,ierr)
  
          ALLOCATE(twidthN(ncbar_partition_1,jcm), STAT=IERR)
          adum = 'twidthN'
          CALL CHECK(adum,ierr)
  
          ALLOCATE(ppdnN(ncbar_partition_1,jcm), STAT=IERR)
          adum = 'ppdnN'
          CALL CHECK(adum,ierr)
  
          ALLOCATE(pslopeN(ncbar_partition_1,jcm), STAT=IERR)
          adum = 'pslopeN'
          CALL CHECK(adum,ierr)
  
          ALLOCATE(pyintN(ncbar_partition_1,jcm), STAT=IERR)
          adum = 'pyintN'
          CALL CHECK(adum,ierr)
  
          ALLOCATE(indtN(ncbar_partition_1,jcm), STAT=IERR)
          adum = 'indtN'
          CALL CHECK(adum,ierr)
          
          ALLOCATE(delhN(ncbar_partition_1,jcm), STAT=IERR)
          adum = 'delhN'
          CALL CHECK(adum,ierr)
          
          ALLOCATE(rllenN(ncbar_partition_1,jcm), STAT=IERR)
          adum = 'rllenN'
          CALL CHECK(adum,ierr)     
          ...
          WRITE(filename(11:),intform) n_partition
          OPEN(718,FILE=filename,FORM='formatted',STATUS='unknown') 
          WRITE(*,*) 'Finish the second part computation component bar ', n_partition
      ...
      !          WRITE(543,*) ((nline(1,i,j),i=1, noub), j=1,ncob)
          ...
          adum = 'delh-deal'
          CALL CHECK(adum,ierr)
          WRITE(*,*) 'Finish the third part computation component bar ', n_partition
  ...
  ! The x- coordinates for all troughs remain the same.
        DO i_partition = 0, ncbar_partition - 1
          ...
          IF (iiii < nocb) THEN
            ...
            ALLOCATE(aaa(ncbar_1,jcm,nt), STAT=IERR)
            adum = 'aaa'
            CALL CHECK(adum,ierr)
      
            ALLOCATE(bbb(ncbar_1,jcm,nt), STAT=IERR)
            adum = 'bbb'
            CALL CHECK(adum,ierr)
      
            ALLOCATE(ccc(ncbar_1,jcm,nt), STAT=IERR)
            adum = 'ccc'
            CALL CHECK(adum,ierr)
      
            ALLOCATE(ddd(ncbar_1,jcm,nt), STAT=IERR)
            adum = 'ddd'
            CALL CHECK(adum,ierr)
            nq1 = 0
            nq2=0
